...
default_dof_pos=np.array([0.,-0.6,1.0 ,0,
                 0.,0.6,-1.,0,
                 0.,-0.6,1.,0,
                -0.,0.6,-1.,0], dtype=np.double)#默认角度需要与isacc一致
torque_limits = np.array([20.0, 55.0, 55.0, 20.0, 55.0, 55.0, 20.0, 55.0, 55.0, 20.0, 55.0, 55.0])
velocity_limits = np.array([40.0, 40.0, 25.0, 40.0, 40.0, 25.0, 40.0, 40.0, 25.0, 40.0, 40.0, 25.0])
...
class Sim2simCfg(A1RoughCfg):
    ...
    class sim_config:
        ...
        mujoco_model_path = '/home/ubuntu/isaac/new_rc/legged_gym/resources/robots/M20/M20_mjcf/mjcf/M20.xml'
    ...
    class robot_config:
        ...
        kps = np.array(80, dtype=np.double)#PD和isacc内部一致
        wheel_kps = np.array(0, dtype=np.double)#PD和isacc内部一致

        kds = np.array(2, dtype=np.double)
        wheel_kds = np.array(0.5, dtype=np.double)
...
if __name__ == '__main__':
    ...
    encoder_model_path = "/home/ubuntu/isaac/new_rc/legged_gym/onnx/encoder_z_input.onnx"
    policy_model_path = "/home/ubuntu/isaac/new_rc/legged_gym/onnx/legged.onnx"
    ...
    for _ in tqdm(range(int(Sim2simCfg.sim_config.sim_duration*10/ Sim2simCfg.sim_config.dt)), desc="Simulating..."):
        ...
        wheel_id = [3,7,11,15]
        wheel_mask = np.zeros(16, dtype=bool)
        wheel_mask[wheel_id] = True
        if 1:
            # 1000hz ->50hz
            if count_lowlevel % Sim2simCfg.sim_config.decimation == 0:

                obs = np.zeros([1, 57], dtype=np.float32) #1,57           
                gravity_vec =  np.array([0., 0., -1.], dtype=np.float32)

                proj_gravity = quat_rotate_inverse(quat,gravity_vec)
                # dq = np.clip(dq, -velocity_limits, velocity_limits)  # 按URDF限幅裁剪转速
                dof_err = q-default_dof_pos
                dof_err[wheel_id] = 0 
                obs[0, 0] = omega[0] *Sim2simCfg.normalization.obs_scales.ang_vel*0.7
                ...
                print("proj_gravity",proj_gravity[:])
                ...
                obs[0, 9:25] = dof_err * Sim2simCfg.normalization.obs_scales.dof_pos #g关节角度顺序依据修改为样机
                ...
                # --- Isaac Gym _compute_torques 逻辑 ---
                action_scale = 0.25
                vel_scale = 10

                dof_err = default_dof_pos - q
                dof_err[wheel_id] = 0

                actions_scaled = action * action_scale
                actions_scaled[wheel_id] = 0

                vel_ref = np.zeros(16, dtype=np.double)
                vel_tmp = action * vel_scale
                vel_ref[wheel_id] = vel_tmp[wheel_id]

                kp = float(Sim2simCfg.robot_config.kps)
                kd = float(Sim2simCfg.robot_config.kds)
                wheel_kp = float(Sim2simCfg.robot_config.wheel_kps)
                wheel_kd = float(Sim2simCfg.robot_config.wheel_kds)

                tau = np.zeros(16)
            # 非轮子: P term = kp * (actions_scaled + dof_err), D term = kd * (vel_ref - dq)
            tau[~wheel_mask] = kp * (actions_scaled[~wheel_mask] + dof_err[~wheel_mask]) \
                                + kd * (vel_ref[~wheel_mask] - dq[~wheel_mask])
            # 轮子: P term = 0 (wheel_kps=0, actions_scaled=0, dof_err=0), D term = kd * (vel_ref - dq)
            tau[wheel_mask] = wheel_kp * (actions_scaled[wheel_mask] + dof_err[wheel_mask]) \
                            + wheel_kd * (vel_ref[wheel_mask] - dq[wheel_mask])
            ...
            tau = np.zeros(16)  # 16 个自由度

            tau[~wheel_mask] = pd_control(target_q[~wheel_mask], q[~wheel_mask], Sim2simCfg.robot_config.kps,
                            target_dq[~wheel_mask], dq[~wheel_mask], Sim2simCfg.robot_config.kds)  # Calc torques
            tau[wheel_mask] = pd_control(np.zeros(4), np.zeros(4), Sim2simCfg.robot_config.wheel_kps,
                            np.zeros(4), dq[wheel_mask], Sim2simCfg.robot_config.wheel_kds) 
